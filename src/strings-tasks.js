...
function repeatString(str, times) {
  if (str && times >= 0) return str.repeat(times);
  return '';
}
...
function formatTime(minutes, seconds) {
  return `${minutes > 9 ? minutes : minutes.toString().padStart(2, '0')}:${
...
function encodeToRot13(str) {
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    if ('ABCDEFGHIJKLM'.includes(str[i].toUpperCase())) {
      result = `${result}${String.fromCharCode(str.charCodeAt(i) + 13)}`;
    } else if ('NOPQRSTUVWXYZ'.includes(str[i].toUpperCase())) {
      result = `${result}${String.fromCharCode(str.charCodeAt(i) - 13)}`;
    } else {
      result = `${result}${str[i]}`;
    }
